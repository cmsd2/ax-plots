...
Status: proposed
Scope: aximar (transport), vscode-extension (renderer), Maxima core (one
small hook), the new `maxima-events` package.

This doc defines the **protocol** — the events channel that flows
structured messages from the Maxima kernel to embedding hosts (aximar
today, potentially others later). It is the foundation that lets us
retire aximar's current sentinel-based stdout-parsing pipeline.
...
This is one of four docs in the design suite:

1. [animate.md](animate.md) — the immediate `animate()` work
   (paradigm 2, precomputed frames). No infrastructure required.
2. [reactive-views.md](reactive-views.md) — the north-star
   architecture. The events channel is Component 6's foundation.
3. **This doc** — the kernel events protocol.
4. [streaming.md](streaming.md) — the first consumer of the protocol:
   per-view streaming with a SUNDIALS PoC.

## Summary

Add a third communication channel from the Maxima kernel to the
frontend: **fd 3** (inheritable pipe) from Maxima to aximar, carrying
newline-delimited JSON envelopes describing structured kernel events.
Aximar forwards each envelope to the vscode-extension as an MCP
notification. The extension forwards to the renderer via VS Code's
notebook renderer messaging API.

The envelope grammar catalogues every kind of kernel event aximar
...
### What aximar reconstructs from stdout today

`aximar/crates/aximar-core/src/maxima/` (~4700 LOC across the module)
contains substantial machinery dedicated to inferring kernel events
from textual output:

| Aximar code | What it's doing | Cost |
|-------------|-----------------|------|
...
| Aximar pattern today | Event type that retires it |
|----------------------|------------------------------|
| `__AXIMAR_EVAL_END__` sentinel injection + scan | `eval_begin` / `eval_end` envelopes |
| `__AXIMAR_VARS__` / `__AXIMAR_VARS_END__` | `vars` envelope |
| `__AXIMAR_LABEL__` + linenum print | `eval_result` envelope (carries `output_label` field) |
...
in-process (Lisp thread in Maxima); aximar is out-of-process (Maxima
...
3. **Deep**: shared `maxima-events` package becomes the substrate for
   both. `maxima_mcp` is the MCP-over-HTTP transport; aximar is the
...
| Leg | Transport | Why |
|-----|-----------|-----|
| Maxima → aximar | fd 3 (inheritable pipe), JSON-lines | Binary-safe, zero escaping, POSIX-standard, no new IPC machinery |
| aximar → extension | MCP `notifications/*` over existing HTTP | Reuses the existing connection; MCP SDK already supports it |
| extension → renderer | VS Code `createRendererMessaging` postMessage | Native VS Code API; iframe-safe |

## Maxima → aximar: the fd 3 channel
...
aximar at spawn time and held open for the lifetime of the session. Maxima
...
aximar's port, auth token, or HTTP. It just writes lines. This keeps the
...
write handle via an env var (`MAXIMA_EVENTS_FD`) set by aximar at spawn.
...
`ax-plots/ax-plots.mac` (`ax__float_to_json`, `ax__float_list_to_json`,
`ax__json_obj`) — extracted into a shared package to avoid duplication.
...
| `$aximar_stream_enabled` boolean global | `$file_output_append` — Maxima-callable `defmvar` flag controlling I/O behaviour | `src/globals.lisp:1480` |
...
- **`dribble`-based transcript in `$writefile`
  ...
  running headless under aximar).
- **Maxima init hooks.** There are none, as confirmed by the survey
  — no `*output-init-hooks*` or similar registry. The reactive-views
  vision section's mention of a "session-init hook" was wishful
  thinking; we should follow Maxima's actual idiom of editing
  `init-cl.lisp` directly. See [§ Patch 1](#patch-1-extend-server-lisps-alternate-i-o-setup-to-add-a-side-channel) below for the
  reframe.

## Aximar side: spawn + reader + MCP notification push

### Spawn change

`McpProcessManager` (in aximar; mirrors the TS-side
...
For each session, aximar spawns an async task that reads `stream_reader`
...
Every envelope carries an implicit `session_id` field added by aximar
...
responds (via an MCP tool call back to aximar) with which mime types it
...
```json
{
  ...
  "lisp": "SBCL 2.4.10",
  "packages": ["numerics", "ax-plots", "numerics-sundials"],
...
on aximar, includes the renderer's accepted mime types:
...
aximar infers this from prompt patterns. The envelope replaces it
explicitly:
...
`show(x)` Maxima primitive, by libraries like `ax-plots` calling
...
For a Plotly figure from `ax_draw2d(...)`:
...
A structured error event. Replaces aximar's current regex-and-grace-
...
the same way aximar feeds it today. The `stdin_request` envelope (see
below) is the cleaner future where the renderer pops up an input UI
and sends `stdin_response` back through aximar; for the PoC we keep
...
- **Refuse.** Aximar returns an error to the new evaluation request
  ("kernel is at the debugger; exit it first via the debugger panel").
  Simple, honest about state.
- **Auto-abort.** Aximar sends an abort command (`:a` to dbm,
...
Today this is a hard problem for aximar — the kernel blocks waiting
for input that aximar has no clean way to deliver. The envelope makes
it explicit:
...
value)` that aximar delivers to the kernel by writing to a dedicated
fd (separate from the cancel fd 4 — see [§ Cancellation](#cancellation)).
`kind` is one of `"string" | "expression" | "debugger_command"`.

### `vars`

Variable list snapshot (replaces `__AXIMAR_VARS__` /
`__AXIMAR_VARS_END__` sentinels at `protocol.rs:147`).
...
- **Coalescing happens in aximar, not the kernel.** A tight loop
  printing 10,000 lines produces 10,000 envelopes from the kernel;
  aximar's reader is free to coalesce adjacent same-(`stream`, `mime`,
...
up, aximar's read buffer fills and the kernel's `force-output` blocks,
...
aximar means even pathological loops sustain thousands of envelopes/sec
...
### Library use — `ax-plots` migration

Today's `ax__emit_plotly` (`ax-plots/ax-plots.mac:1419`) writes the
Plotly JSON to a temp file and prints the path; aximar's parser scans
...
Today, aximar wraps user code with `{user code}; tex(%); print(LABEL);
...
`stdin_response(request_id, value)` that aximar delivers to the kernel
...
1. **Load-only**: the `maxima-events` package is loaded by
   `maxima-init.lisp`, by aximar's spawn wrapper, or by the user. No
...
| SUNDIALS streaming PoC | Yes | Already designed as a `.mac` file in `numerics-sundials`. |
...
1. **Aximar-driven spawn**: aximar invokes Maxima with
   `maxima --very-quiet --batch-string='load("maxima-events.mac")$' --userdir=...`,
   ensuring the package is loaded before the first user evaluation.
   *Recommended for the PoC* — keeps the dependency at the embedding
   host's discretion.
2. **User's `maxima-init.lisp`**: a one-line addition. Works for
   non-aximar consumers (CLI users, custom hosts).
...
aximar can still parse with the legacy sentinel pipeline as fallback.
...
- A shared contract — `MAXIMA_EVENTS_FD` and `*maxima-events-out*`
  become a public Maxima embedding API rather than aximar-private.
...
The current pipeline at `aximar/crates/aximar-core/src/maxima/` (~4700
...
| Aximar stops injecting `__AXIMAR_EVAL_END__` and `__AXIMAR_READY__` | ~40 LOC at `protocol.rs:42, 96` |
| Aximar stops scanning text output for those sentinels | ~30 LOC at `process.rs` (eval-end detection loop) |
| Aximar stops stripping sentinels (and LaTeX-escaped variants) from `text_output` | ~50 LOC at `parser.rs:273, 290` |

User-visible win: user code containing `__AXIMAR_` strings is no
longer confused with kernel framing. Estimate: 1 week, mostly
test updates.
...
| Aximar stops capturing stdout for `EvalResult.text_output`; derives it from `output` envelopes instead | ~60 LOC at `process.rs` (output-buffer-merge logic) |
...
| Aximar stops injecting `tex(%);` and `print("__AXIMAR_LABEL__"...)` | ~20 LOC at `protocol.rs:42, 96` |
| `ax-plots` emits `display` envelopes for plots instead of writing temp files | ~30 LOC at `ax-plots.mac:1419` |
| Aximar stops scanning for `.plotly.json` / `.png` / `.svg` paths and reading temp files | ~150 LOC at `parser.rs:464–515` |
...
biggest single retirement** — eliminates ~250 LOC of aximar parser
machinery.
...
| Aximar stops `ERROR_MARKERS` regex scanning | ~40 LOC at `process.rs:430` and `debugger.rs` |
...
| Aximar stops `(dbm:N>` and SBCL `0]` prompt detection on partial reads | ~100 LOC at `process.rs:572, 623, 636` and `debugger.rs` |
...
| Aximar relays the response to the kernel via a dedicated fd | ~50 LOC new |
...
| Aximar stops injecting `__AXIMAR_VARS__` / `__AXIMAR_VARS_END__` | ~30 LOC at `protocol.rs:147` |

Cleanup step. Estimate: 2 days.

### Total accounting

If all seven phases land, aximar's `maxima/` module shrinks by an
...
envelope *and* the old `tex(%)` printout, for example, with aximar
...
2. **Authorisation on the events channel.** Anyone with read access
   ...
   aximar over a network socket.
3. **Multiple frontends on one session.** The renderer-side
   subscription tables (see
   [reactive-views.md § Component 3](reactive-views.md#component-3-subscription-model--expression-level-reactivity))
   are per-frontend. If two frontends connect to one aximar session,
   each should get every envelope; their subscription tables filter
   independently. Aximar's MCP notification should fan out — already
   the default for MCP notifications.
...
2. **Aximar spawn + fd 3 reader + MCP notification push.** Includes
   the envelope schema validation. Testable against a mock kernel that
   writes canned envelopes. *~1 week.*
3. **vscode-extension controller notification handler +
   renderer-messaging plumbing.** Testable against a mock aximar that
...
|-------|---------|--------|
| **M3** — `display` + `eval_result` mime bundles | ~250 LOC of aximar parser code (temp-file scanning, LaTeX extraction, `tex(%)` injection); structured plot output | ~2 weeks |
...
| **M1** — eval lifecycle envelopes | The `__AXIMAR_EVAL_END__` sentinel pipeline | ~1 week |
...
- [animate.md](animate.md) — paradigm-2 animation (sibling design, no
  infrastructure)
- [reactive-views.md § Component 6](reactive-views.md#component-6-streaming--incremental-computation)
  — the north-star fit
- [streaming.md](streaming.md) — first consumer of this protocol;
  streaming envelopes, cancellation, SUNDIALS PoC
- `aximar/crates/aximar-core/src/maxima/protocol.rs:10–12` — current
  sentinel definitions (to be retired)
- `aximar/crates/aximar-core/src/maxima/process.rs:430, 572, 623, 636`
  — current prompt/error pattern detection (to be retired)
- `aximar/crates/aximar-core/src/maxima/parser.rs:273, 464` — current
  text-output sentinel stripping and temp-file path scanning (to be
  retired)
- `vscode-extension/src/notebook/mcpClient.ts` — current MCP transport
  (line 9, 221) and spawn logic (line 173) to extend with fd 3
