...
aximar's reader, the renderer-messaging plumbing) and the general
...
1. [animate.md](animate.md) — the immediate `animate()` work
   (paradigm 2, precomputed frames). No streaming required.
2. [reactive-views.md](reactive-views.md) — the north-star
   architecture. Streaming is Component 6 of six.
3. [kernel-events.md](kernel-events.md) — the events-channel protocol
   that streaming sits on top of. **Read that first** for transport
   and general envelope conventions; this doc references it
   throughout.
4. **This doc** — streaming envelopes, the cancellation design, and
   the SUNDIALS PoC.
...
- A SUNDIALS-based proof of concept demonstrating end-to-end
  ...
  `numerics-sundials/lisp/sundials/cvode-wrapper.lisp:650, 718`)
...
- **Transport.** Maxima writes newline-delimited JSON envelopes to
  fd 3 (`MAXIMA_EVENTS_FD`); aximar reads them and pushes via MCP
  `notifications/maxima_kernel_event`; the vscode-extension forwards
  to the renderer via `createRendererMessaging`. See
  [kernel-events.md § Maxima → aximar fd 3 channel](kernel-events.md#maxima--aximar-the-fd-3-channel).
...
If the renderer can't keep up (slow JS, hidden tab), aximar buffers up
...
| `\"accumulating\"` (e.g. `table_append`, `optimization_history`) | block the writer (`force-output` to fd 3 blocks when aximar's read buffer is full) |
...
Aximar writes one byte to fd 4 to signal cancel. The byte's value
...
In `numerics-sundials/lisp/sundials/cvode-wrapper.lisp:683`, the RHS
...
`.mac` file `numerics-sundials/streaming.mac`, loaded via
`load("numerics-sundials-streaming")`. The existing `numerics-sundials`
...
load("numerics-sundials-streaming")$
...
**Lisp addition** (in `numerics-sundials/lisp/sundials/cvode-wrapper.lisp`,
...
(defun $np_cvode_step_one (handle &optional t-stop)
  ...
  (let ((t-target (or (and t-stop (coerce (maxima::$float t-stop)
    ...
    (multiple-value-bind (t-actual y-list events-list)
        (numerics-sundials::np-cvode-step-internal-onestep
...
(`cvode-wrapper.lisp:604`) with one line changed: the `itask` argument
...
- **`rk`, `rkf45` in `dynamics`.** They could stream with a 5-LOC
  callback parameter (see [reactive-views.md § Component 6](reactive-views.md#component-6-streaming--incremental-computation)),
  but we don't touch them in this PoC. SUNDIALS demonstrates the
  protocol; once accepted, retrofitting is mechanical.
...
- **The `views` package** (Component 5 of reactive-views). The PoC
  uses `view_id` strings directly; the eventual `view` object will
  carry the view_id and the kind. The PoC's hand-allocated view_ids
  are forward-compatible with that.
...
| Pipe → aximar read → MCP push | <500µs | 200–400µs (single host, localhost) |
...
Backpressure: aximar's read buffer is 64KB by default; each envelope
...
1. Renderer posts `set_signal(zeta_id, 0.3)` → controller → aximar →
...
3. **Authorisation on the cancel pipe.** Anyone with write access to
   ...
   expose aximar over a network socket.
4. **Multiple frontends on one session.** The renderer-side
   subscription tables (see [reactive-views.md § Component 3](reactive-views.md#component-3-subscription-model--expression-level-reactivity))
...
5. **Pause and resume.** Should `cancel_view` actually *pause* with
   ...
   `np_cvode_step` again from the current state); pausing the kernel
...
2. **Aximar streaming-aware reader extensions.** Backpressure policy
...
- [kernel-events.md](kernel-events.md) — the protocol foundation
  this doc builds on
- [animate.md](animate.md) — paradigm-2 frames (sibling design, no
  streaming required)
- [reactive-views.md § Component 6](reactive-views.md#component-6-streaming--incremental-computation)
  — the north-star fit for streaming
- `numerics-sundials/lisp/sundials/cvode-wrapper.lisp` — the existing
  persistent-handle API at line 650 (the foundation the PoC builds on)
- `numerics-sundials/lisp/sundials/cvode.lisp` — the CFFI bindings at
  line 74 (CVode + `itask` parameter, used by Mode 2)
